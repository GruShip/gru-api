...
import com.tech.dream.db.repository.ProductBrandRepository;
...
import com.tech.dream.model.ProductBrandDTO;
...
public class ProductBrandService {
	...
	public Object create(ProductBrandDTO dto, SessionDTO session) {
		ErrorDTO error = createValidation(dto, session);
		if (error != null) {
			return error;
		}
		ProductBrand entity = convertToEntity(dto);
	...
	public Object update(ProductBrandDTO dto, SessionDTO session) {
		ErrorDTO error = updateValidation(dto, session);
		if (error != null) {
			return error;
		}
		repository.updateProductBrand(dto.getId(), dto.getName(), dto.getCompany(), dto.getActive()!=null?dto.getActive():true, dto.getDesc());
		return dto;
	}

	public Object delete(ProductBrandDTO dto, SessionDTO session) {
		ErrorDTO error = deleteValidation(dto, session);
		if (error != null) {
			return error;
		}
		repository.deleteProductBrand(dto.getId());
		return true;
	}

	public ProductBrand convertToEntity(ProductBrandDTO dto) {
	...
	private ErrorDTO deleteValidation(ProductBrandDTO dto, SessionDTO session) {
		...
		if(!CompanyType.MARKETPLACE.equalsIgnoreCase(session.getCompanyType())) {
			return new ErrorDTO(HttpStatus.FORBIDDEN, DisplayModuleNames.USER+" does not have access to create "+DisplayModuleNames.PRODUCTBRAND);
		}
		if (repository.existsProductBrandById(dto.getId())<=0) {
			return new ErrorDTO(HttpStatus.BAD_REQUEST, DisplayModuleNames.PRODUCTBRAND+" does not exists for given id.");
		}
		if (productRepository.existsProductByProductBrandId(dto.getId())>0) {
			return new ErrorDTO(HttpStatus.BAD_REQUEST, DisplayModuleNames.PRODUCT+"'s are mapped to given "+DisplayModuleNames.PRODUCTBRAND);
		}
		return null;
	}

	private ErrorDTO createValidation(ProductBrandDTO dto, SessionDTO session) {
		...
		if (repository.existsProductBrandByName(dto.getName())>0) {
			return new ErrorDTO(HttpStatus.BAD_REQUEST, DisplayModuleNames.PRODUCTBRAND+" already exists for give name.");
		}
		return null;
	}

	private ErrorDTO updateValidation(ProductBrandDTO dto, SessionDTO session) {
		if (StringUtils.isEmpty(dto.getId())) {
			return new ErrorDTO(HttpStatus.BAD_REQUEST, "Id is mandatory and cannot be null.");
		}
		if(!CompanyType.MARKETPLACE.equalsIgnoreCase(session.getCompanyType())) {
		...
		if (repository.existsProductBrandByNameAndNotId(dto.getName(), dto.getId())>0) {
			return new ErrorDTO(HttpStatus.BAD_REQUEST, DisplayModuleNames.PRODUCTBRAND+" already exists for give name");
		}
		return null;
	}
	...
	public ResultDTO list(SessionDTO session, PagingSortSearchDTO filters) {
		...
		try {
			String query = "select pb.id, pb.name, pb.company, pb.`desc`, pb.active "
			+ " from productbrand pb "
			+ " where pb.removed = FALSE";
			...
			if (filters.getSearch() != null){
				for (SearchQueryDTO sDto: filters.getSearch()){
					...
					String fieldDatatype = getProductBrandFieldDataType(sDto.getSearchField());
		...
		List<ProductBrandDTO> dtoList = new ArrayList<>();
		if(datalistResult!=null && datalistResult.size()>0) {
			for(Object[] dataResult : datalistResult) {
				ProductBrandDTO dto = this.fillProductBrandDTO(dataResult);
				dtoList.add(dto);
			}
		}
	...
	public Object get(SessionDTO session, Long id) {
		...
		List<Object[]> datalistResult = repository.findProductBrandDataById(id);
		ProductBrandDTO dto = null;
		if(datalistResult!=null && datalistResult.size()>0) {
			Object[] dataResult = datalistResult.get(0);
			dto = this.fillProductBrandDTO(dataResult);
		}
		return dto;
	}
	...
	private ProductBrandDTO fillProductBrandDTO(Object[] result) {
	...
	private String getProductBrandFieldDataType(String column) {
		switch (column) {
			case "id":
			case "active":
